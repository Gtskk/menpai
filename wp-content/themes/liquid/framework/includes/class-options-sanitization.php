...
/* Skin */
add_filter( 'of_sanitize_skin', 'of_sanitize_enum', 10, 2 );
...
/* Tile */
add_filter( 'of_sanitize_tile', 'of_sanitize_upload');
...
/* Sorter */
function sanitize_sorter( $input ) {
	$output = array();
	$i = 0;
	if(is_array($input)):
	foreach($input as $input){
		$i++;
		if(isset($input['title'])) $output[$i]['title'] = apply_filters( 'sanitize_text_field', $input['title'] );
		if(isset($input['hook'])) $output[$i]['hook'] = $input['hook'];
		if(isset($input['icon'])) $output[$i]['icon'] = $input['icon'];
		if(isset($input['image'])) $output[$i]['image'] = apply_filters( 'of_sanitize_upload', $input['image'] );
		if(isset($input['url'])) $output[$i]['url'] = apply_filters( 'sanitize_text_field', $input['url'] );
		if(isset($input['content'])) $output[$i]['content'] = apply_filters( 'of_sanitize_textarea', $input['content'] );
		if(isset($input['autop'])) $output[$i]['autop'] = apply_filters( 'of_sanitize_enum', $input['autop'], 10, 2 );
	}
	endif;
	return $output;
}
add_filter( 'of_sanitize_sorter', 'sanitize_sorter' );
