	...
	static function optionsframework_tabs() {
		...
		foreach ( $options as $value ) {
			...
			if ( $value['type'] == "heading" ) {
				...
				$menu .= '<li><a id="options-group-'.  $counter . '-tab" class="' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '">';
				if($value['icon']!='') $menu .= '<i class="fa '.$value['icon'].'"></i>';
				$menu .= esc_html( $value['name'] ) . '</a></li>';
			}
		}
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			case 'select':
				...
				$output .= '<select class="of-input chosen-select"'.$multi.' name="' . esc_attr( $name ) . '" id="' . esc_attr( $value['id'] ) . '">';				
				foreach ($value['options'] as $key => $option ) {
					$selected = '';
					if(isset($value['multi']) && is_array($val)){
			...
			case "skin":
				...
				foreach ($value['options'] as $key => $option) {
					...
					$output .= '<input class="of-input of-radio-skin" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $key ) . '" '. checked( $val, $key, false) .' />';
					$output .= '<span class="of-radio-skin-img '.$selected.'" style="background-color: #'.$option['color'].'"';
					$output .= ' data-color="'.$option['color'].'" data-bgcolor="'.$option['bgcolor'].'" data-bgimg="'.$option['bgimg'].'"';
					$output .= ' onClick="document.getElementById(\''. esc_attr( $id ) .'\').checked = true;"';
					$output .= '></span>';
				}
				break;
			...
			case 'tile':
				...
				foreach ($value['options'] as $key => $option) { 
					...
					$output .= '<input class="of-input of-radio-tile" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $option ) . '" '. checked( $val, $option, false) .' />';
					$output .= '<span id="tile-'.esc_attr( $filename ).'" class="of-radio-tile-img'. $selected .'" style="background: url('.esc_attr( $option ).')" onClick="document.getElementById(\''. esc_attr( $id ) .'\').checked = true;"></span>';
				}
				break;
			...
			// Sortable UI
			case 'sorter':
				$output .= Options_Framework_Sorter::optionsframework_sorter( $value, $val );
				break;
			...
			// Icon
			case "icon":
				$output .= '<input type="hidden" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" id="' . esc_attr( $value['id'] ) . '" value="' . esc_attr( $val ) . '"/>';
				if($val) { $v = explode('|',$val); $icon_class = $v[0].' '.$v[1];}
				$output .= '<span type="button" class="button icon-picker '.$icon_class.'"></span>';
			...
			case 'google_font':
				...
				$output .= '<select class="of-input google_font_select chosen-select" id="'. esc_attr( $value['id'] ) .'" name="'. esc_attr( $option_name . '[' . $value['id'] . ']' ) .'">';
				foreach ($value['options'] as $key => $option) {
					$output .= '<option value="'.$key.'" ' . selected( $val, $key, false ) . ' />'.$option.'</option>';
				} 
				$output .= '</select>';
